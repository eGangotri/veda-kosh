...
const YajurVedaSingleMantra: React.FC<{ mantraRefId: string }> = ({ mantraRefId }) => {
    ...
    const createValuesForMandala = async (adhyayaNo: number, mantraNo: number) => {
        createValues(`2/${adhyayaNo}/${mantraNo}/1`)
    }
    ...
    const mantraCount = getYajurvedaMantraCount() || 0;
    ...
    const generateNumberBoxes = (count: number) => {
        return Array.from({ length: count }, (_, i) => (
            <Button
                ...
                variant="outlined"
                sx={{ m: 0.5 }}
                onClick={(e) => createValuesForMandala(selectedMandala, parseInt(e.currentTarget.textContent || "1"))}
            >
                {i + 1}
            </Button>
        ))
        ...
        <Box sx={{ p: 3, display: 'flex', gap: 3 }}>
            <Paper elevation={3} sx={{ p: 3, width: '30%' }}>
                ...
                <Grid container spacing={1}>
                    {Array.from({ length: mantraCount }, (_, i) => (
                        <Grid item xs={3} key={i}>
                            <Button
                                variant={selectedMantra === i + 1 ? "contained" : "outlined"}
                                fullWidth
                                onClick={() => createValuesForMandala(1, i + 1)}
                                sx={{
                                    aspectRatio: '1',
                                    minWidth: 'unset',
                                    backgroundColor: i === 0 ? '#1a237e' : undefined, // Dark blue for first box
                                    '&:hover': {
                                        backgroundColor: i === 0 ? '#283593' : undefined
                                    }
                                }}
                            >
                                {i + 1}

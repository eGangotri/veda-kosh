...
export function useVedaSearch(
    ...
    try {
      ...
      Object.entries(initialSearchParams).forEach(([key, value]) => {
        if (value !== undefined && value !== null && value !== '') {
          if (key === 'vedaType' && Array.isArray(value) && value.length > 0) {
            // Handle vedaType as a comma-separated list
  ...
  useEffect(() => {
   console.log(`useVedaSearch useEffect: query=${query}, initialSearchParams=${JSON.stringify(initialSearchParams)}, searchTrigger=${searchTrigger}`)
    fetchResults()
